...
compute_gcor_normalized_binom <- function(model0, X, 
  ...
  if ((algorithm=="intercept_only")||intercept_only) {
    ...
    r_plus <- exact_threshold_search_intercept_only(as.numeric(X))
    r_minus <- -r_plus
    ...
    if ((algorithm=="brute_force")||(length(X) <= n_exact)) {
      max_result <- bruteforce_r(Z, X, mode = "max",link = link_fun)
      r_plus <- max_result$r
      if (is_odd_symmetric) {
        r_minus <- -r_plus
      } else {
        min_result <- bruteforce_r(Z, X, mode = "min",link = link_fun)
        r_minus <- min_result$r
      }
      algorithm="brute_force"
    } else { #then "multi_start"
      max_result <- multi_start_r(Z, X, Y_user = model0$y, mode = "max", link = link_fun,
      ...
      if (is_odd_symmetric) {
        ...
        min_result <- multi_start_r(Z, X, Y_user = model0$y, mode = "min", link = link_fun, 
                                    thresholds = algorithm.control$thresholds,
                                    n_random = algorithm.control$n_random, 
                                    max_iter = algorithm.control$max_iter,
                                    topK = algorithm.control$topK, 
  ...
  if (r > 0) {
    normalized_r <- r / r_plus
  } else if (r < 0) {
    normalized_r <- -r / r_minus
  } else {
    normalized_r <- 0
  }
