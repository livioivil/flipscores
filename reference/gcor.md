...
#> (Intercept) -0.14256 -0.91360    2.62144 -0.34851    -0.127    0.734   
#> ZB          -0.18558 -0.50868    1.65785 -0.30683    -0.108    0.660   
#> ZC           1.40981  8.55380    2.58950  3.30326     0.765    0.007 **
#> X           -0.06964 -1.56935    4.70999 -0.33320    -0.117    0.681   
#> ---
...
#> X     ~X -0.1174534 -0.2755351   ~1+ZB+ZC
#> ZC   ~ZC  0.7651942  1.0442471    ~1+ZB+X


gcor(mod, intercept_too=TRUE, normalize=TRUE)
#>                    terms          r        r_n null_model
#> (Intercept) ~(Intercept) -0.1265802 -0.2077374 ~1+ZB+ZC+X
#> ZB                   ~ZB -0.1080979 -0.1532418    ~1+ZC+X
#> ZC                   ~ZC  0.7651942  1.0442471    ~1+ZB+X
#> X                     ~X -0.1174534 -0.2755351   ~1+ZB+ZC
set.seed(123)
dt=data.frame(X=rnorm(20),
...
#> Coefficients:
#>             Estimate    Score Std. Error  z value Part. Cor Pr(>|z|)  
#> (Intercept)  -0.1486  -0.2102     1.1881  -0.1770    -0.067    0.901  
#> ZB          -20.4539  -1.4784     0.7466  -1.9802    -0.530    0.051 .
#> ZC           20.8561   1.8043     0.8180   2.2057     0.615    0.034 *
#> X            -0.4276  -0.3782     0.9574  -0.3951    -0.149    0.747  
#> ---
#> Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
#> 
...
#>    terms          r        r_n null_model
#> ZB   ~ZB -0.5295718        NaN    ~1+ZC+X
#> ZC   ~ZC  0.6146304        NaN    ~1+ZB+X
#> X     ~X -0.1493213 -0.3669599   ~1+ZB+ZC
# Compute for specific terms only
gcor(mod, terms = c("X", "ZC"),normalize=TRUE)
#>    terms          r        r_n null_model
#> X     ~X -0.1493213 -0.3669599   ~1+ZB+ZC
