...
mod=flipscores_glm(Survived~Class*Sex+Age,data=Titanic,family = binomial, score_type = "effective")
summary(mod)
```
...
mod=flipscores_glm(Freq~.+Class*Sex, data=Titanic, family = poisson)
summary(mod)
```
...
mod=flipscores_glm(Volume~Girth*Height, data=trees, family = gaussian)
summary(mod)
```
...
mod=flipscores_glm(speed~dist, data=cars, family = Gamma, score_type = "ortho")
summary(mod)
```
...
# mod=flipscores_glm(Freq~.+Class*Sex, data=Titanic, family = "negbinom")
# summary(mod)
```
...
## clustered units
mod=flipscores_glm(y~x*z,data=D,family = binomial,id=D$id)
summary(mod)
...
mod=flipscores_glm(y~x*z,data=D,family = poisson)
summary(mod)

#### negative binomial
summary(MASS::glm.nb(y~x*z,data=D))
mod=flipscores_glm(y~x*z,data=D,family="negbinom")
summary(mod)
